...
const headers = ["ID", "Customer Name", "Phone", "Address", "Date", "Action"];
...
export default function OrderTable() {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(`/api/proxy/v1/customers?${query}`);
      const json = await res.json();
      const raw = json.data ?? [];

      const formatted: Customer[] = raw.map((c: any) => ({
        id: c.id,
        name: c.name,
        email: c.email,
        phone: c.phone,
  ...
  const handleOpenDetail = (order: Customer) => {
  ...
  const handleOpenEdit = (order: Customer) => {
    setSelectedOrder(order);
        ...
        <GenericTable
          ...
          renderRow={(order, index) => (
            <>
              ...
              <TableCell className="px-4 py-3 text-start text-gray-500 text-start dark:text-gray-400">{order.address}</TableCell>
