...
import { DealDetailModal } from "../deal/component/DealDetailModal";

const headers = ["ID", "Name", "Phone", "Status", "Needs", "Company", "Date", "Action"];
...
interface Lead {
  ...
  phone: string;
  company: string;
  status: 'new' | 'contacted' | 'qualified' | 'unqualified' | 'in_progress' | 'lost';
  address: string;
  user_id: number;
  needs: string;
  date: string;
...
export default function OrderTable() {
  ...
  const [selectedOrder, setSelectedOrder] = useState<Lead | null>(null);
  const [selectLead, setSelectLead] = useState<Lead | null>(null);
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(`/api/proxy/v1/leads?${query}`);
      const json = await res.json();
      const raw = json.data ?? [];

      const formatted: Lead[] = raw.map((c: any) => ({
        id: c.id,
        name: c.name,
        email: c.email,
        phone: c.phone,
    ...
    try {
      ...
      if (selectedOrder != null) {
        if (payload.status == 'qualified') {
          ...
          url = `/api/proxy/v1/leads/process/${selectedOrder?.id}`
          method = 'POST'
        } else {
          url = `/api/proxy/v1/leads/${selectedOrder?.id}`
          method = 'PUT'
        }
      } else {
        url = `/api/proxy/v1/leads`
        method = 'POST'
      }
  ...
  const handleOpenDetail = (order: Lead) => {
    setSelectLead(order);
  ...
  const handleOpenEdit = (order: Lead) => {
            ...
            <button
              onClick={async () => {
                ...
                const token = document.cookie
                  ...
                  ?.split('=')[1];
                fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/v1/leads/export?${query}`, {
                  method: 'GET',
                  headers: {
        ...
        <GenericTable
          ...
          renderRow={(order, index) => (
            <>
              ...
              <TableCell className="px-4 py-3 text-start text-gray-500 text-start dark:text-gray-400">{order.phone}</TableCell>
              ...
              <TableCell className="px-4 py-3 text-start text-gray-500 text-start dark:text-gray-400">{order.company}</TableCell>
